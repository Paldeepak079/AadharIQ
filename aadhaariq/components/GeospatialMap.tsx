...
import { MapPin, ArrowLeft, ZoomIn, Globe } from 'lucide-react';
...
const GeospatialMap: React.FC<MapProps> = ({ lang, onSelect }) => {
  ...
  const districtMapData = currentStateForDistricts && districtData[currentStateForDistricts.toUpperCase()]
    ? Object.entries(districtData[currentStateForDistricts.toUpperCase()]).map(([name, data]) => ({
      name,
      lat: data.lat,
      lng: data.lng,
      ...
      child_enrollments: data.child_enrollments,
